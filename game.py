...
class Game:
    ...
    def play(self):
        ...
        while True:
            ...
            # Check and return player choice until its valid
            ...
            self.board.update_board(
                player_choice // self.board.height,
                player_choice % self.board.width,
            ...
            if self.check_if_win(
                player_choice // self.board.height, player_choice % self.board.width
            ):
                print_welcome_game()
                self.board.print_board()
                print(
    ...
    def check_and_return_valid_choice(self, player_choice):
        ...
        while not self.board.is_place_valid(
            player_choice // self.board.height, player_choice % self.board.width
        ):
            print("Wrong input or place is already taken.")
            player_choice = input("Move > ")
    ...
    def switch_turn(self):
        self.whose_turn_is_it = (
            self.player2 if self.whose_turn_is_it == self.player1 else self.player1
        )

    def check_if_win(self, x, y):
        if self.check_if_row_win(x) or self.check_if_col_win(y):
            return True
        if self.check_if_left_diagonal_win(x, y) or self.check_if_right_diagonal_win(
            x, y
        ):
            return True
        return False
